...
static void copy_sb16_16(uint16_t *dst, int dstride,
...
void av1_cdef_search(YV12_BUFFER_CONFIG *frame, const YV12_BUFFER_CONFIG *ref,
  ...
  for (sbr = 0; sbr < nvsb; sbr++) {
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      int gi;
      int dirinit = 0;
      DECLARE_ALIGNED(32, uint16_t, dst[3][MAX_MIB_SIZE * MAX_MIB_SIZE * 8 * 8]);
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        copy_sb16_16(dst[pli], MAX_MIB_SIZE << bsize[pli], src[pli],
            sbr * MAX_MIB_SIZE << bsize[pli], sbc * MAX_MIB_SIZE << bsize[pli],
            stride[pli], nvb << bsize[pli], nhb << bsize[pli]);
      }
      for (gi = 0; gi < TOTAL_STRENGTHS; gi++) {
        ...
        for (pli = 0; pli < nplanes; pli++) {
          int clpf_damping = 4 - (pli != AOM_PLANE_Y) + (cm->base_qindex >> 6);
          ...
          int ysize = (nvb << bsize[pli]) + OD_FILT_VBORDER * (sbr != nvsb - 1)
              + yoff;
          int xsize = (nhb << bsize[pli]) + OD_FILT_HBORDER * (sbc != nhsb - 1)
              + xoff;
          ...
          od_dering(tmp_dst, in, dec[pli], dir, &dirinit, var, pli, dlist,
                    ...
                    coeff_shift);
          copy_dering_16bit_to_16bit(dst[pli], MAX_MIB_SIZE << bsize[pli], tmp_dst,
