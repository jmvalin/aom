...
void av1_dering_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  int sbr, sbc;
  int nhsb, nvsb;
  od_dering_in dst[OD_BSIZE_MAX*OD_BSIZE_MAX];
  ...
  for (sbr = 0; sbr < nvsb; sbr++) {
    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      for (pli = 0; pli < 3; pli++) {
        od_inbuf_copy(&inbuf[pli][toggle * nhsb * OD_DERING_INBUF_SIZE + in_write],
    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      for (pli = 0; pli < 3; pli++) {
        ...
        od_dering(&OD_DERING_VTBL_C,
                  dst,
                  OD_BSIZE_MAX,
                  &inbuf[pli][!toggle * nhsb * OD_DERING_INBUF_SIZE + in_read],
                  &xd->plane[pli].dst.buf[(sbr - 1) * bsize[pli] * MAX_MIB_SIZE * xd->plane[pli].dst.stride + sbc * bsize[pli] * MAX_MIB_SIZE],
                  xd->plane[pli].dst.stride,
