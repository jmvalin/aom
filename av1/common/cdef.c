  ...
  for (sbr = 0; sbr < nvsb; sbr++) {
    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      level = dering_level_table
          [cm->cdef_strengths[cm->mi_grid_visible[MAX_MIB_SIZE * sbr * cm->mi_stride +
                                   MAX_MIB_SIZE * sbc]
                   ->mbmi.cdef_strength] /
           CLPF_STRENGTHS];
      clpf_strength =
      ...
      uv_level = dering_level_table
          [cm->cdef_uv_strengths[cm->mi_grid_visible[MAX_MIB_SIZE * sbr * cm->mi_stride +
                                   MAX_MIB_SIZE * sbc]
                   ->mbmi.cdef_strength] /
           CLPF_STRENGTHS];
      uv_clpf_strength =
      ...
      if ((level == 0 && clpf_strength == 0 && uv_level == 0 && uv_clpf_strength == 0) ||
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        if (pli) {
          if (chroma_dering) level = uv_level;
          else level = 0;
